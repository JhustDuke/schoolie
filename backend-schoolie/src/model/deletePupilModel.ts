...
export const deletePupilModel = async function ({
	...
	try {
		const [rows]: any = await conn.query(
			`SELECT classes, total_boys, total_girls FROM \`${table}\` WHERE id=1`
		);
		...
		const index = pupilArr.findIndex(function (p: any) {
			return (
		...
		if (gender === "female" && totalGirls > 0) totalGirls -= 1;
		if (gender === "male" && totalBoys > 0) totalBoys -= 1;

		const updated = JSON.stringify(parsedClasses);
		await conn.query(
			`UPDATE \`${table}\` SET classes=?, total_girls=?, total_boys=? WHERE id=1`,
			[updated, totalGirls, totalBoys]
		);

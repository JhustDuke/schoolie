...
import { PluginSpecificConfiguration } from "@hapi/hapi/lib/types/plugin";
import {
	MergeType,
	ReqRef,
	ReqRefDefaults,
...
import {
	...
	RouteExtObject,
	Server,
} from "@hapi/hapi/lib/types/server";
import { Lifecycle, Json, HTTP_METHODS } from "@hapi/hapi/lib/types/utils";
...
export interface RouteOptionsAccess {
	 ...
	 */
	scope?:
		| MergeType<InternalRouteOptionType, RouteOptionTypes>["Scope"]
		| undefined;
	 ...
	 */
	strategies?:
		| MergeType<InternalRouteOptionType, RouteOptionTypes>["Strategy"][]
		| undefined;
...
export interface RouteOptionsPayload {
	 ...
	 */
	compression?:
		| { [P in keyof ContentDecoders]?: Parameters<ContentDecoders[P]>[0] }
		| undefined;
 ...
 */
export type RouteOptionsPreArray<Refs extends ReqRef = ReqRefDefaults> =
	RouteOptionsPreAllOptions<Refs>[];

/**
 * For context [See docs](https://github.com/hapijs/hapi/blob/master/API.md#-routeoptionspre)
 */
export type RouteOptionsPreAllOptions<Refs extends ReqRef = ReqRefDefaults> =
	| RouteOptionsPreObject<Refs>
	| RouteOptionsPreObject<Refs>[]
...
export interface CommonRouteProperties<Refs extends ReqRef = ReqRefDefaults> {
	 ...
	 */
	compression?:
		| { [P in keyof ContentEncoders]?: Parameters<ContentEncoders[P]>[0] }
		| undefined;
...
export interface RouteSettings<Refs extends ReqRef = ReqRefDefaults>
	extends CommonRouteProperties<Refs> {
 ...
 */
export interface RouteOptions<Refs extends ReqRef = ReqRefDefaults>
	extends CommonRouteProperties<Refs> {
	/**
...
export interface RulesOptions<Refs extends ReqRef = ReqRefDefaults> {
	validate: {
		schema?:
			| ObjectSchema<MergeRefs<Refs>["Rules"]>
			| Record<keyof MergeRefs<Refs>["Rules"], Schema>
			| undefined;
...
export interface RulesProcessor<Refs extends ReqRef = ReqRefDefaults> {
	(rules: MergeRefs<Refs>["Rules"] | null, info: RulesInfo): Partial<
		RouteOptions<Refs>
	> | null;
}

type RouteDefMethods = Exclude<
	HTTP_METHODS | Lowercase<HTTP_METHODS>,
	"HEAD" | "head"
>;
...
export interface ServerRoute<Refs extends ReqRef = ReqRefDefaults> {
	...
	options?:
		| RouteOptions<Refs>
		| ((server: Server) => RouteOptions<Refs>)
		| undefined;
